...
export class EaoWordEditControls extends HTMLElement {
  ...
  updateWord = (e) => {
    ...
    console.log('updateWord', this.word.value, this.word.data.value);
  ...
  connectedCallback() {
    ...
    this.valueInput.addEventListener('focus', (e) => {
      this.valueInput.select();
    });

    this.speakerSelect.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        this.speakerSelect.value = this.word.speaker.id;
        this.speakerSelect.blur();
      }
    });

    this.discardChangesButton.addEventListener('click', this.discardChanges);
    this.updateWordButton.addEventListener('click', this.updateWord);
  }
  ...
  get discardChangesButton() {
    return this._shadow.querySelector('#discardChangesButton');
  }
  get updateWordButton() {
    return this._shadow.querySelector('#updateWordButton');
  }
  ...
  get template() {
    ...
    <fieldset>
      ...
      <button id="updateWordButton">Update</button>
    </fieldset>
    `;
  }
}
